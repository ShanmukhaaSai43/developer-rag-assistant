...
def replay_trace(trace: dict, ai_client=None, max_retries: int = 3) -> dict:
    """
    ...
    if not replayed_output and last_err:
        replayed_output = f"ERROR: Replay failed ({last_err})"
    ...
    return {
        ...
        "audit_notes": "All required fields (prompt_version, retrieved_chunks with scores/content, model parameters, raw_prompt) were fully reconstructed from the trace record alone."
    }
